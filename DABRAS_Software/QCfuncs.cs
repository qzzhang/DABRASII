...
namespace DABRAS_Software
{
    public partial class FormQC
    {
        ...
        private void BG_BackgroundThreadFinished()
        {
            ...
            {
                MessageBox.Show("Background calibration test failed.");
                this.bgPassed = false;
        ...
        private void AB_BackgroundThreadFinished()
        {
            ...
            if (QCAB.WasTestPassed() && QCAB.WasTestCompleted())
            {
                ...
                {
                    RadionuclideFamily RF = ListOfFamily.Find(x => x.GetName() == "Am-241");
                ...
                {
                    RadionuclideFamily RF = ListOfFamily.Find(x => x.GetName() == "Sr-90");
            ...
            {
                MessageBox.Show("Calibration test failed.");
                this.abPassed = false;
                this.frmParent.EnableRSCForm(false);
            }
        ...
        public void updFamilyAndSource(List<RadionuclideFamily> famList, List<Radioactive_Source> srcList)
        {
            ...
            foreach (RadionuclideFamily rf in famList)
            {
                foreach (Radioactive_Source src in srcList)
                {
                    ...
                    {
                        if (DateTime.Compare(dt, src.GetDailyCalibratedTime()) < 0)
                        {
                            dt = src.GetDailyCalibratedTime();
                            curr_src = src;
                        }
                    }
                }
    ...
    public class QCBackgroundListener
    {
        ...
        private void tryReconnectDABRAS()
        {
            if (this.DABRAS.VCP_Instance != "")
            {
                try
                {
                    while (!this.DABRAS.Get_Coms(DABRAS.VCP_Instance))
                        Thread.Sleep(100);
                }
                finally
                {
            ...
            try
            {
                while (!DABRAS.IsDataReady() && !ShouldStop)
                {
                    ...
                    if (!DABRAS.IsConnected())
                    {
                        ...
                        //this.tryReconnectDABRAS();//try to re-connect seamlessly!
                    }
                }
                return;
            }
    ...
    public class QCAlphaBetaListener
    {
        ...
        private string BuildToolTipText(double avga, double avgb,bool lessAlphaLo, bool greaterAlphaHi, bool lessBetaLo, bool greaterBetaHi)
        {
            ...
            if (lessAlphaLo)
            {
                resnBuilder.AppendFormat("Average Alpha - AlphaBackground = {0} is less than the alpha lower limit {1}", StaticMethods.RoundToDecimal(avga, 2), Lo);
                resnBuilder.Append(Environment.NewLine);
            }
            if (greaterAlphaHi)
            {
                resnBuilder.AppendFormat("Average Alpha - AlphaBackground = {0} is higher than the alpha higher limit {1}", StaticMethods.RoundToDecimal(avga, 2), Hi);
                resnBuilder.Append(Environment.NewLine);
            }
            if (lessBetaLo)
            {
                resnBuilder.AppendFormat("Average Beta - BetaBackground = {0} is less than the beta lower limit {1}", StaticMethods.RoundToDecimal(avgb, 2), Lo);
                resnBuilder.Append(Environment.NewLine);
            }
            if (greaterBetaHi)
            {
                resnBuilder.AppendFormat("Average Beta - BetaBackground = {0} is higher than the beta higher limit {1}", StaticMethods.RoundToDecimal(avgb, 2), Hi);
                resnBuilder.Append(Environment.NewLine);
            }
            return resnBuilder.ToString();
        }
