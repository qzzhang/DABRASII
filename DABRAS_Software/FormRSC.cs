...
namespace DABRAS_Software
{
    public partial class FormRSC : Form
    {
        ...
        private void New_Count_Button_Click(object sender, EventArgs e)
        {
            ...
            if (this.UsingAnnual())
            {
                FullDataResults["NewColumn", 9].Value = BackgroundSource.GetAnnualCalibratedTime();
                FullDataResults["NewColumn", 10].Value = BackgroundSource.GetAnnualCalibratedTimespan();
                FullDataResults["NewColumn", 11].Value = StaticMethods.RoundToDecimal(BackgroundSource.GetAnnualAlphaCPM(), 2);
                FullDataResults["NewColumn", 12].Value = StaticMethods.RoundToDecimal(BackgroundSource.GetAnnualBetaCPM(), 2);
            }
            else if (UsingDaily())
            {
                FullDataResults["NewColumn", 9].Value = BackgroundSource.GetDailyCalibratedTime();
                FullDataResults["NewColumn", 10].Value = BackgroundSource.GetDailyCalibratedTimespan();
                FullDataResults["NewColumn", 11].Value = StaticMethods.RoundToDecimal(BackgroundSource.GetDailyAlphaCPM(), 2);
                FullDataResults["NewColumn", 12].Value = StaticMethods.RoundToDecimal(BackgroundSource.GetDailyBetaCPM(), 2);
            }
        ...
        {
            /*Remove the 4 rows that contain the following data:
